...
///
/// 实现选 ScriptProcessorNode（deprecated 但 Chrome/Safari/Firefox 全都还支持）。
/// AudioWorklet 更现代但要单独 worklet 文件 + Vite bundling——移植自 jarvis-pet
/// 保持同款省工程量。downsample 用整除比 3:1（48k→16k）取样，无 anti-alias
/// filter，语音内容上 alias 几乎不可闻。
///
/// 用法：
...
///   rec.stop()
///
/// 浏览器默认 AudioContext sampleRate 通常 48000（部分安卓机 44100）；
/// resample 按实际 sampleRate 整除比取整，非整除比有小漂移但 ASR 不敏感。
