...
fn stage_write_creates_staging_dir() {
    ...
    assert!(
        path.exists(),
        "staging ROLE.md 应当存在: {}",
        path.display()
    );
    assert!(
        path.ends_with("painter.staging/ROLE.md"),
        "实际路径 = {}",
        path.display()
    );
}
...
fn approve_renames_staging_to_active_atomic() {
    ...
    assert!(active.ends_with("painter/ROLE.md"), "{}", active.display());
...
fn reject_removes_staging_without_touching_active() {
    ...
    std::fs::write(dir.path().join("scribe/ROLE.md"), simple_skill("scribe")).unwrap();
    ...
    assert!(dir.path().join("scribe/ROLE.md").exists(), "active 被误删");
}
...
fn list_all_distinguishes_active_and_staging() {
    ...
    std::fs::write(dir.path().join("dev/ROLE.md"), simple_skill("dev")).unwrap();
    stage_write(dir.path(), "painter", &simple_skill("painter")).expect("stage");
...
fn approve_replaces_existing_active_with_bak() {
    let dir = tempfile::tempdir().unwrap();
    // 已入册（旧文件名 SKILL.md 也要兼容）
    std::fs::create_dir_all(dir.path().join("scribe")).unwrap();
    std::fs::write(dir.path().join("scribe/ROLE.md"), "旧版本\n").unwrap();
    // 改版入榜
    stage_write(dir.path(), "scribe", "新版本\n").expect("stage");

    approve(dir.path(), "scribe").expect("approve");
    let active_body = std::fs::read_to_string(dir.path().join("scribe/ROLE.md")).unwrap();
...
/// M3.2 · list_all 兼容仍叫 SKILL.md 的旧目录——用户没跑 migrate_user_dir 时也能列到。
#[test]
fn list_all_accepts_legacy_skill_md_files() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(dir.path().join("legacy")).unwrap();
    std::fs::write(dir.path().join("legacy/SKILL.md"), simple_skill("legacy")).unwrap();

    let entries = list_all(dir.path()).expect("list");
    assert_eq!(entries.len(), 1, "旧 SKILL.md 应仍被列出");
    assert_eq!(entries[0].role, "legacy");
    assert!(entries[0].path.ends_with("legacy/SKILL.md"));
}
