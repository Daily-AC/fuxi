...
///
/// 桌宠场景：常驻 WS + 麦克风永远开着采样上推。server 检到「玄女」下行
/// wake 事件，桌宠收到后触发 voice flow（开 ASR 走 STT → intervene → 玄女
/// reply + TTS）。
///
/// 鉴权：URL 走 `?token=...`（桌宠 webview WS 不能 set header；wake server
/// 已 patch 接受 query token，commit `feat(wake): query token 兼容`）。
///
