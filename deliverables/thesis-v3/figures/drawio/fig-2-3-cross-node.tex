...
\node[numBubble] at ($(home_dist.east)+(5mm,6mm)$) {1};
\draw[flowEvent, very thick, dash pattern=on 3pt off 2pt]
  ($(home_dist.east)+(0,2mm)$) -- node[above=2pt, font=\scriptsize, color=fuxiBlue, sloped, midway] {HMAC + ssh tunnel}
  (worker1_relay.west);

\node[numBubble] at ($(home_dist.east)+(5mm,-6mm)$) {2};
\draw[flowEvent, very thick, dash pattern=on 3pt off 2pt]
  ($(home_dist.east)+(0,-2mm)$) to[bend right=15] node[below=2pt, font=\scriptsize, color=fuxiBlue, sloped, pos=0.4] {/dist/* HMAC}
  (worker2_relay.west);
