...
本实现的传输层选择了 JSON-RPC 2.0 而非 REST。所有方法（沿用早期 A2A binding 命名 \texttt{agent/getCard}、\texttt{tasks/send}、\texttt{tasks/get}、\texttt{tasks/cancel}、\texttt{tasks/sendSubscribe}，对应当前规范的 \texttt{GetExtendedAgentCard}、\texttt{SendMessage}、\texttt{GetTask}、\texttt{CancelTask}、\texttt{SubscribeToTask}）都通过同一个 \texttt{POST /a2a} 端点提交，靠请求体里的 \texttt{method} 字段路由。代码~\ref{lst:jsonrpc} 给出信封类型。
