...
\subsection{面向 Fuxi 场景的 A2A 协议核心闭环实现（论文核心贡献）}

A2A（Agent-to-Agent）是 Google 在 2025 年提出并由 Linux Foundation 托管的开放 agent 互通协议\cite{a2aproject2025a2a}，其规范级别等价于 MCP 在工具调用领域的位置\cite{anthropic2024mcp}。这两个协议构成了 LLM 时代 agent 生态的两条主轴：MCP 解决"agent 如何调用工具"，A2A 解决"agent 如何调用其他 agent"。当前 A2A 生态已包含 Python、Go、Java、JavaScript、.NET 与 Rust（\texttt{a2a-rs}）六个语言的官方 SDK，Rust 生态内还有 \texttt{a2a-client}、\texttt{a2a-types} 等第三方 crate 提供独立的协议封装。本工作没有把贡献建立在「Rust 生态空白」之上，而是面向伏羲的本地优先编排场景，从零实现一份与事件总线、玄女—门客编排层、反向 WebSocket sandbox 同源演进的 A2A 风格核心 RPC 闭环，并把规范已有的 \texttt{input-required} 中间态映射到 Fuxi 平台级的人工介入信号上。这一节展开该实现的具体决策。
...
设计这个 crate 时面临的第一个根本问题是"实现到什么粒度"。A2A 规范涵盖了 agent 自描述、任务生命周期、流式推送、artifact 分块上传、push notification、双向 TLS、版本协商等若干主题，全部实现工程量巨大。我们采取了"覆盖 fuxi 场景所需闭环、明确简化项"的务实策略：覆盖 5 条核心路径（\texttt{agent/getCard} / \texttt{tasks/send} / \texttt{tasks/sendSubscribe} / \texttt{tasks/get} / \texttt{tasks/cancel}）及对应的 \texttt{TaskState} 子集（\texttt{Submitted} / \texttt{Working} / \texttt{InputRequired} / \texttt{Completed} / \texttt{Failed} / \texttt{Canceled}，参照规范子集，\texttt{InputRequired} 是规范已有的 interrupted state）、\texttt{Message} / \texttt{Part} / \texttt{Artifact} 层次结构以及 SSE 流式分块；明确简化掉 push notification（fuxi 内网部署，无 webhook 基础设施）、双向 TLS（信任边界在进程而非网络）、自动重连（上层 \texttt{A2ARouter} 负责）、版本协商。这一覆盖范围对应 fuxi 平台所需的核心闭环——足以承载本工作所有的 agent discovery、task send、task stream、task query 与 task cancel 五条主路径；与当前官方规范的完整 wire-level 互通验证留待后续工作（详见 §6.2 局限）。crate 体积也因此控制在 1\,063 行核心代码，便于审阅。
...
\texttt{InputRequired} 是 fuxi-a2a 中最重要的一个状态。本实现参照的早期 A2A draft 中 \texttt{TaskState} 只包含 \texttt{Submitted/Working/Completed/Failed/Canceled} 五个核心态，全部是"agent 自治"语义；当前官方规范在此基础上扩展了 \texttt{TASK\_STATE\_INPUT\_REQUIRED} 与 \texttt{TASK\_STATE\_AUTH\_REQUIRED} 两个 interrupted state（非终态、可恢复），并新增 \texttt{TASK\_STATE\_REJECTED} 这一拒绝执行的终态。伏羲的工作模式是"人在回路"——门客在某些决策点必须等用户确认（例如 reviewer 看完代码后是否合并），因此本实现提前显式建模了 \texttt{InputRequired = "input-required"} 状态，让对端编排器（玄女）能感知"这个 task 不是失败，而是等人工"。代码~\ref{lst:taskstate} 给出 fuxi-a2a 当前实现的状态枚举（仍未覆盖 \texttt{AuthRequired}、\texttt{Rejected}，详见 §6.2）。
...
本实现的传输层选择了 JSON-RPC 2.0 而非 REST。所有方法沿用早期 A2A binding 命名（\texttt{agent/getCard}、\texttt{tasks/send}、\texttt{tasks/get}、\texttt{tasks/cancel}、\texttt{tasks/sendSubscribe}）：其中 task send / get / cancel / stream 在语义上分别接近当前规范的 \texttt{SendMessage}、\texttt{GetTask}、\texttt{CancelTask}、\texttt{SendStreamingMessage} / \texttt{SubscribeToTask} 路径；agent card 获取路径属于早期 discovery binding，未实现当前规范中 authenticated extended card 的完整语义。所有方法都通过同一个 \texttt{POST /a2a} 端点提交，靠请求体里的 \texttt{method} 字段路由。代码~\ref{lst:jsonrpc} 给出信封类型。
...
设计精妙处在于：\texttt{tasks/sendSubscribe} 和其他四个方法共用同一个 \texttt{POST /a2a} URL，但响应类型不同——前者返回 \texttt{text/event-stream}，后者返回 \texttt{application/json}。Axum 的 \texttt{IntoResponse} trait 让两种返回类型在同一个函数签名下统一，调用方只需检查 \texttt{Content-Type} 即可决定如何消费。伏羲选择保留早期 binding 中的「单端点 + method 分发 + SSE 升级」形式（备选方案是「双端点 + 显式 \texttt{/a2a/stream}」），因为单端点能简化 reverse proxy 配置，nginx 只需转发一个 location；当前官方 binding 的完整端点与方法形态不在本工作覆盖范围内。\texttt{KeepAlive::default()} 启用心跳防止中间代理（cloudflare 等）以"60 秒空闲"为由切断长连接。
...
\subsubsection{InputRequired 状态：A2A 规范语义在 Fuxi 平台的映射}

如前所述，\texttt{InputRequired} 是 A2A 规范已有的 interrupted task state（对应当前规范 ProtoJSON 枚举 \texttt{TASK\_STATE\_INPUT\_REQUIRED}；规范同等级的 interrupted 分支还有 \texttt{TASK\_STATE\_AUTH\_REQUIRED}，而 \texttt{TASK\_STATE\_REJECTED} 是终态），伏羲在工程层面把它从协议字段提升为平台级人工介入信号。其语义是：当 agent 在执行过程中遇到必须由人工决策的分支（典型例子是 reviewer 看完 PR 后不能自己 merge），不发 \texttt{Failed} 也不发 \texttt{Completed}，而是发一条 \texttt{TaskState::InputRequired} 状态事件并暂停推送，等外部编排器（玄女）通过 task send 再发一条用户 message 才能继续。这一设计在 fuxi 内部对应 \texttt{Task::PendingApproval} 与 \texttt{ShelfStatus::AwaitingInput}，让整体调度器与多端观测能正确把「在等用户」的门客剥离出「在工作」的资源池。这种"人在回路"显式建模在多 agent 协同系统中不是可有可无的奢侈品，而是工程必要——否则 agent 在等待人工时只能伪装成"超长 working"，与真正的死循环无法区分。

更深一层看，伏羲选择把 \texttt{InputRequired} 提升为平台级人工介入信号是设计哲学的微缩展示。本工作所参考的早期 A2A draft 默认 agent 是自治闭环系统——要么成功要么失败；伏羲的核心立场是"agent 是工具不是替代品"，必须给人工决策点显式建模，因而在 fuxi-a2a 中提前显式实现了这一中间态。值得说明的是，A2A 当前官方规范也正式将 \texttt{InputRequired} 纳入 interrupted task state 集合，与本实现的工程动机一致——本工作的工程价值不在于为规范新增状态，而在于把这一规范状态从协议字段提升为 Fuxi 内部 \texttt{Task::PendingApproval} 与 \texttt{ShelfStatus::AwaitingInput}，使整体调度器、跨节点 \texttt{NodeLoadProvider} 与多端观测都能正确把"在等用户"的门客剥离出"在工作"的资源池。这一立场延伸到伏羲的多个其它设计——玄女永远有知情权（公理 \#2）、deliverable 必须显式 produce 而非自动推断、跨节点任务的 \texttt{InputRequired} 会被 republish 到 home 节点的 EventBus 让玄女感知。这种"协议形态承载产品哲学"的设计模式与 SMTP 通过 EHLO 扩展机制（RFC 5321）在长期协议演进中持续吸纳新语义类似——区别在于本工作做的是把已纳入规范的语义在工程层落地，不是为规范立法。

回看整个 \texttt{fuxi-a2a} crate：1\,063 行核心库 + 258 行测试，覆盖 5 条核心路径、6 种 \texttt{TaskState} 子集、3 种 \texttt{Part} 类型；端到端测试 \texttt{roundtrip\_all\_methods} 验证完整的 client-server 调用闭环，\texttt{roundtrip\_subscribe\_stream} 专门验证 SSE 帧在 TCP chunk 边界上的正确切割。这是 \texttt{fuxi} 平台为本地优先 Agent 协作场景定制的一份 A2A 风格协议适配层，是本工作的核心贡献之一。与生态内官方 SDK \texttt{a2a-rs} 以及 \texttt{a2a-client}、\texttt{a2a-types} 等第三方 crate 处在不同定位上：前者以通用 SDK 与协议封装为目标，本 crate 则与 \texttt{fuxi-events} 事件总线、玄女—门客编排层、反向 WebSocket sandbox 同源演进，并把 A2A 规范已有的 \texttt{input-required} 中间态映射到平台级人工介入信号上。从复杂度看，它跨越了 wire format、JSON-RPC envelope、HTTP server、SSE streaming、reverse client 五个层次的协议工程；从平台价值看，它把 fuxi 的可观测性约束与编排层假设直接编码进协议实现。本 crate 当前的测试覆盖证明它在 Fuxi 所需的 5 条核心路径上形成了稳定闭环，并已在伏羲的 cc 适配器、codex 适配器以及一个作者维护的外部 Python 参考 endpoint 上做端到端互通；与官方 \texttt{a2a-rs} SDK 的完整 wire-level 互操作验证留待后续工作（详见 §6.2 局限）。
