...
A2A（Agent-to-Agent）是 Google 在 2025 年提出并由 Linux Foundation 托管的开放 agent 互通协议\cite{a2aproject2025a2a}，其规范级别等价于 MCP 在工具调用领域的位置\cite{anthropic2024mcp}。这两个协议构成了 LLM 时代 agent 生态的两条主轴：MCP 解决"agent 如何调用工具"，A2A 解决"agent 如何调用其他 agent"。当前 A2A 生态已包含官方与第三方多语言 SDK，其中 Rust 生态已有 a2a-rs、a2a-client、a2a-types 等若干第三方 crate 提供独立的协议封装。本工作没有把贡献建立在「Rust 生态空白」之上，而是面向伏羲的本地优先编排场景，从零实现一份与事件总线、玄女—门客编排层、反向 WebSocket sandbox 同源演进的 A2A 核心闭环，并在状态机层扩展 \texttt{InputRequired} 人工介入语义。这一节展开该实现的具体决策。
