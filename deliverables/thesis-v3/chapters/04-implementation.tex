...
\texttt{InputRequired} 是 fuxi-a2a 中最重要的一个状态。本实现参照的早期 A2A draft 中 \texttt{TaskState} 只包含 \texttt{Submitted/Working/Completed/Failed/Canceled} 五个核心态，全部是"agent 自治"语义；当前官方规范已正式纳入 \texttt{InputRequired}、\texttt{AuthRequired}、\texttt{Rejected} 三个 interrupted/non-terminal 状态。伏羲的工作模式是"人在回路"——门客在某些决策点必须等用户确认（例如 reviewer 看完代码后是否合并），因此本实现提前显式建模了 \texttt{InputRequired = "input-required"} 状态，让对端编排器（玄女）能感知"这个 task 不是失败，而是等人工"。代码~\ref{lst:taskstate} 给出 fuxi-a2a 当前实现的状态枚举（仍未覆盖 \texttt{AuthRequired}、\texttt{Rejected}，详见 §6.2）。
