...
第四章给出了 Fuxi 的实现细节，本章在该实现基础上对系统进行多维度的测试与结果分析。本章测试目标分为正确性、实时性、吞吐扩展性、参数敏感性以及通讯层压力承受能力五大类，覆盖系统从底层事件总线到顶层任务编排的完整链路。所有定量实验在 Apple Silicon (M-series) 处理器、macOS 25.0 操作系统、Rust stable 工具链与 Tokio 多线程运行时之上完成；吞吐与扩展性指标采用 5-run median 抑制冷启动与系统抖动造成的长尾，延迟指标采用 500 个独立样本统计 p50 与 p99；事件总线压测每个 cell 持续 5 秒稳态后再采样。完整原始数据见仓库 \texttt{deliverables/thesis-v2/benchmarks/v2-2026-05-07.md}，本章正文引用的所有定量数字均可在该报告中追溯。本章不仅汇报数字，更着重对每一组数字给出物理意义层面的解释——即「为什么是这个数字」、「它在系统设计上意味着什么」以及「与文献中相近系统在数量级上是否自洽」，以避免实验章退化为单纯的数据登记。
...
需要补充说明的是，本章关注的核心问题是「Fuxi 的事件驱动通讯底座在自身的设计目标——AI Agent 工作负载——下表现如何」，而非「Fuxi 与同类系统横向对比谁更快」。这两类问题本质不同：前者关心系统是否兑现了设计承诺，对应公理 3「真实时，不轮询」、公理 5「SQLite 是单一真相源」等内部不变量的可验证性；后者关心市场定位，对应论文之外的工程决策。本章选择前者作为论证主线，因此实验设计上更注重内部一致性与可重复性，而非横向竞速。
...
按公式 (2-3)，单个任务的端到端时间可写作 $T_{e2e} = T_{exec} + T_{comm}$，其中 $T_{comm}$ 是 Fuxi 框架引入的通讯与调度开销，包含 axum HTTP 往返（约 4--6 ms，含 TLS 握手在本机环回为 0、HMAC 验证、JSON 编解码与中间件栈）、Tokio 任务调度（约 2--3 ms，含跨线程唤醒与异步 future poll）、SQLite 状态写入（约 1--2 ms，WAL 模式下 fsync 已合并）以及 Worker 拉取轮询（poll\_ms 平均一半的等待期望）。这些开销在不同任务粒度下的占比差异即为损耗递减的根源。
...
延迟测试针对系统的两条关键通讯路径：任务派发链路与跨节点事件流。结果如表 \ref{tab:latency} 所示。
...
\textbf{与文献中相近系统的对比}。MapReduce 在 Google 的原始论文中报告 1800 mapper 下扩展效率约 76\%--82\%\cite{dean2004mapreduce}；本节 16 worker 下报告 83.5\%——两者数量级一致，但本章规模远小，不能直接断言「Fuxi 优于 MapReduce」。该对照仅说明 Fuxi 在小规模下的扩展行为与公开系统的扩展规律相符，没有出现单点瓶颈或数据局部性破坏。Spanner 在跨数据中心场景报告的扩展效率受地理延迟主导，与本章实验的本机场景不可比；Raft 在多副本场景下扩展效率因为共识通讯而显著低于无状态系统\cite{ongaro2014raft}——Fuxi 的任务派发不需要跨副本一致性（每个任务有唯一 worker 处理），因此扩展上限不受共识协议限制，这一架构选择的代价是不支持任务级别的故障容错（worker 崩溃后任务需要重派而非自动迁移），属于第三章关于「以工作量复用替代复制」的设计取舍。
...
定量结果可以总结为五个数字。第一，8 worker × 10 ms 模拟任务条件下达到 \textbf{654.66 tasks/s} 的吞吐量，工程损耗稳定在 18\% 量级，且在 100 ms 与 1000 ms 任务粒度下损耗降至 2\% 与 0.3\%；第二，Worker 数 1→16 范围内扩展效率始终保持在 \textbf{79.7\%--83.5\%} 之间，未出现明显衰减；第三，任务派发 p50 延迟为 \textbf{32.97 ms}、p99 为 37.32 ms，跨节点事件流 p50 仅 \textbf{0.07 ms}、p99 为 0.15 ms，两者相差三个数量级；第四，事件总线 sustainable 容量为 \textbf{1.6 M events/s}，超过 Fuxi 实际工作负载三个数量级；第五，端到端延迟在 1000 ms 任务粒度下，通讯开销占比仅 \textbf{3.2\%}，工程上可忽略。这些数据共同验证了第二章关于「事件驱动通讯底座可同时兼顾吞吐、实时性与可追溯性」的设计假设，也为第六章总结与展望提供了定量依据。

定性层面，本章的分析揭示了 Fuxi 系统设计的三项核心取舍。其一，\textbf{事件驱动 vs 轮询}：在小粒度任务下，事件驱动相比朴素轮询能把损耗从 50\% 量级压缩到 18\%，是 Fuxi 实时性的根基。其二，\textbf{广播 fan-out 的 O(1) 性质}：Tokio broadcast 让事件总线在 64 sub × 10k events/s 下零丢帧，让所有观察端可以「看似免费地」订阅，构成系统可观测性的基础。其三，\textbf{相对于 AI Agent 工作负载的工程余量}：通讯开销在 LLM 推理粒度下占比可忽略，意味着 Fuxi 不是一个为通讯极限设计的低延迟交易系统，而是为「让 AI Agent 之间的协作既实时又可追溯」量身定做的基础设施。这三项取舍共同回答了本论文反复追问的核心问题——「为何选择 Rust + Tokio + 事件驱动，而非 Python + 轮询 + REST」——本章的数字给出了工程层面的支撑。
