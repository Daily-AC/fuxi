...
\textbf{尾部比与稳定性}。任务派发的 p99/p50 比值为 37.32/32.97 = 1.13，说明派发链路尾部抖动可控；事件总线广播 p99/p50 比值为 0.15/0.07 = 2.14，绝对差仅 0.08 ms。事件流的相对比值看上去更大，但这并非「事件流尾部更不稳定」的反例——在亚毫秒量级，OS 时钟分辨率（macOS 上 \texttt{Instant} 实测有效精度约 41 ns，但 \texttt{SystemTime} 经 ns→us 截断后是 1 us）即足以让相对比值放大；从绝对值看，0.08 ms 的尾部增量相对任何上层应用的反应时间都可忽略。
