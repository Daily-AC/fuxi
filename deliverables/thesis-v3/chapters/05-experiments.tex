...
从表 \ref{tab:throughput} 可观察到三组层次分明的现象。\textbf{第一组}：当任务粒度为 10 ms 时，1、4、8 个 Worker 的吞吐分别为 81.83、331.67 与 654.66 tasks/s，与理论上限的差距稳定在 17\%--19\% 之间。\textbf{第二组}：当任务粒度上升到 100 ms 时，损耗骤降到 2.1\%--2.3\%。\textbf{第三组}：当任务粒度进一步增加到 1000 ms 时，损耗已降至 0.2\%--0.4\% 量级。三段任务粒度下损耗递减的物理原因可由上述 $T_{e2e}=T_{exec}+T_{comm}$ 简化记法的分量构成给出。
