...
\textbf{贡献一：与 fuxi 平台同源演进的 A2A 风格协议适配层。} \texttt{fuxi-a2a} crate 共 1321 行，实现了覆盖 agent discovery、task send、task stream、task query 与 task cancel 五条核心路径的轻量协议闭环，沿用早期 A2A JSON-RPC binding（方法命名 \texttt{agent/getCard}、\texttt{tasks/send} 等）。该实现并不以替代官方 \texttt{a2a-rs} SDK 或完整覆盖当前 A2A 规范为目标——后者在 a2a-rs / a2a-client / a2a-types 等 crate 中已有可比的覆盖；本 crate 的差异化定位在于与 \texttt{fuxi-events} 事件总线、玄女—门客编排层与反向 WebSocket sandbox 的深度集成，以及把 A2A 规范已有的 \texttt{TASK\_STATE\_INPUT\_REQUIRED} 人工介入语义（在本工作沿用的早期 binding 中编码为 \texttt{"input-required"}）映射到 \texttt{Task::PendingApproval} 与 \texttt{ShelfStatus::AwaitingInput}，使人工介入成为平台级可观测事件。\texttt{fuxi-a2a} 与 MCP~\parencite{anthropic2024mcp}在层次上互补——A2A 处理 Agent 间协作契约，MCP 处理模型与工具的调用规范。
...
\textbf{工程兑现（不作为独立贡献项列出）。} 上述五项设计在 13 crate、约 8.25 万行 Rust 代码、1363 个单元测试与 27 个集成测试文件中同时成立，CI 门禁要求 \texttt{cargo fmt --check}、\texttt{cargo clippy -D warnings} 与 \texttt{cargo test --workspace} 三项全绿才能合并。第五章在 Apple Silicon 平台上对吞吐量、延迟、参数敏感性与事件总线压力进行了多维测量：剥离 LLM 推理后的合成任务基准下（每任务以 10 ms sleep 模拟），8 worker 配置下达到 \textbf{665.00 tasks/s} 的调度路径吞吐，Worker 扩展至 16 时吞吐增长至 \textbf{1288.24 tasks/s}（1$\rightarrow$16 worker 区间扩展效率介于 80.5\%--83.1\%，dispatcher 在该区间未饱和）；任务派发链路 p50 延迟 \textbf{32.93 ms}、p99 为 36.66 ms，进程内事件总线广播 p50 仅 \textbf{0.08 ms}、p99 为 0.26 ms；事件总线在 64 subscriber 并发订阅、\textbf{10\,000 events/s} 持续负载下保持零丢帧（更高速率下的拐点分析见 §5.7）。这些合成基准衡量的是编排与通讯层在剥离 LLM 推理后的纯通讯吞吐上限，真实 LLM Agent 任务的端到端吞吐受单次推理（典型 1--30 s）主导。除定量数据外，系统已在作者维护的 ERP 项目中作为日常协作平台运行，承担需求拆解、代码改动与 PR 评审等多轮对话型工作，迈过了「日常使用品」意义上的最低可用线——这是上述五项设计在真实代码与真实业务上同时成立的工程证据。
...
\textbf{横向 baseline 对比缺失。} 第五章选择以「理论上限对照」作为主要参照，没有与 LangChain、AutoGen、CrewAI 等公开 Agent 编排框架做实测吞吐对比。理由是跨语言、跨抽象层级的 baseline 比较容易让数字失去工程意义——差异主要来自 Python 与 Rust、同步与异步等系统底层因素，而非编排层设计取舍。但这意味着本文未能给出 Fuxi 在主流框架坐标系下的相对位置。在统一负载、统一任务语义与统一观测口径之上的横向 baseline 是后续工作之一。

\textbf{A2A 协议规范跟进与官方 SDK 互通验证待补。} A2A 规范在 2025--2026 年间仍处于快速演进阶段，当前官方规范已切到 PascalCase 方法集（\texttt{SendMessage}、\texttt{SendStreamingMessage}、\texttt{GetTask}、\texttt{CancelTask}、\texttt{SubscribeToTask}），\texttt{Part} 也使用 one-of 字段语义而非 tagged union。本文 \texttt{fuxi-a2a} 实现沿用早期 A2A JSON-RPC binding（方法命名 \texttt{agent/getCard}、\texttt{tasks/send} 等，\texttt{Part} 使用 \texttt{\#[serde(tag="type")]} tagged union），覆盖了 Fuxi 所需的 5 条核心协作路径，但尚未与当前官方 \texttt{a2a-rs} SDK 做完整 wire-level 互操作验证，也未完全覆盖最新规范中的全部方法、状态（如 \texttt{auth-required}、\texttt{rejected}）与 binding 细节。后续工作将把该 crate 升级为当前官方规范兼容层，或显式维护 legacy binding 适配器，并补 \texttt{a2a-rs} 互通测试。
