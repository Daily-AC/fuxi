...
- **CC model fallback 也改空串**（2026-05-04 用户 home 实测）：原 `DEFAULT_MODEL_FALLBACK = "sonnet"` 在用户的 claude.ai 账号下别名解到 `claude-sonnet-4-6-1m` → "API Error: Extra usage is required for 1M context"，玄女整轮失败。同 codex 一样，`crates/fuxi-agent-cc/src/config.rs::DEFAULT_MODEL_FALLBACK = ""` 不传 `--model` 让 cc 走账号默认（用户主账号 = opus-4-7-1m）。`smoke-p0.sh` 加了反回归断言。
...
- **加 EventKind 新变体**必同步 6 处（task #8 实测——`UsageReport`/`XuannvContextWatermark`/`XuannvHandoffWritten` 三个变体撞了 6 处编译错）：`events/store.rs::kind_tag` + `firehose/hub.rs::kind_tag` + `firehose/tui.rs::summarize + color_for` + `cli/subcommands.rs::event_summary` + 持久化/round-trip 测试。clippy `-D warnings` 一次性报四处缺失。
...
- **shutdown_agent 必须豁免玄女**（2026-04-20 `1e6db4e`）：`IdleGcTask` 10 分钟 idle 会误杀 xuannv（她对 GC 是普通 agent）。治本：`Fuxi::shutdown_agent` 开头比对 `xuannv_id()`，命中 warn + 返 Ok 静默 noop。**任何新 shutdown 路径（`fuxi kill --id` / 将来的 worker pool rebalance）都从这个方法走，不绕旁路**。只有 `Fuxi::shutdown()`（整体下线）能关玄女。**例外**：task #8 `Fuxi::shutdown_xuannv_for_handoff(id, reason)` 显式绕豁免——用户主动交接才走，命名带 `_for_handoff` 让 grep 一眼看清「不要在别处调用」。
...
- **cc spawn 别强塞 `--session-id`/`--resume`**（2026-05-11 `2173ebe`）：cc 2.1.114+ 在 `--sdk-url` SDK 模式下把 `--session-id <new-uuid>` 也当 strict resume——session 不存在就 stderr「No conversation found with session ID: ...」立死，cc 进程变 zombie，但 fuxi 因 `ws_bridge` 只收到 "Connection reset by peer" **没改 status**，玄女永久"假活 Idle" 用户消息全黑洞。stdio 模式（无 `--sdk-url`）`--session-id <new-uuid>` 仍允许新建——所以 ssh 手测可能误判这条 cc 行为正常。修法：四处 spawn 路径（`daemon.rs::spawn_by_role` / `xuannv_bootstrap::ensure_xuannv` / `repl.rs` 启动玄女 / `xuannv_handoff::spawn_with_prelude`）一律 `session_id: None, resume_session_id: None`，让 cc 自己生成；`ws_bridge.rs:287` 已经 peek `system/init` 拿到 sid。代价是丢 cc 端 session resume——玄女每次重启 = fresh cc session，但 fuxi events.db 历史完整（PWA / jarvis 历史不丢）。**deploy 提示**：home 上 fuxi binary 装两份，`/home/e0-7/.local/bin/fuxi`（systemd 用）+ `/home/e0-7/.cargo/bin/fuxi`（PATH 优先），cp 时**两份都要换**。
- **GPT-SoVITS NLTK `averaged_perceptron_tagger_eng` 缺失**（2026-05-11 `6869aaf`）：玄女回话含英文词（"Jarvis"/"say"）时 sovits 走英文 POS tagger 撞 400 `Resource 'averaged_perceptron_tagger_eng' not found`，jarvis 客户端降级回系统 TTS，用户感知「第一句派蒙音色 OK 第二句变机械女声」。`install.sh` 装的是老 NLTK 名 `averaged_perceptron_tagger`，NLTK 4+ 改名带 `_eng` 后缀不命中。补：`python -m nltk.downloader averaged_perceptron_tagger_eng cmudict` + `sudo systemctl restart sovits.service`。**新装 sovits 必跑这条**。
- **fuxi-wake-server xfyun SDK session 卡死**（2026-05-11 home 实测）：jarvis 喊「玄女」无任何反应，wake server log `session alreay started, err=18310`。讯飞 SDK 的 session lifecycle 没释放（前一个 client 没 graceful disconnect），新 client 连进来 SDK 拒。**临时修**：`sudo systemctl restart fuxi-wake.service`。**长期治本**：fuxi-wake-server 加 client disconnect 时 force-reset xfyun session 兜底，避免讯飞自身没清的状态污染下一个 client。
- **mac shell `HTTPS_PROXY` 干扰 ssh ProxyCommand**（2026-05-11 实测）：`~/.ssh/config` 的 `Host home` 用 cloudflare-dns DOH 查 A 记录然后 nc。如果 mac shell 设了 `HTTPS_PROXY`（用户日常 dev 常见 Clash/Surge HTTP proxy），DOH `curl` 走代理偶发 SSL 35 失败 → DNS 返空 → ssh 认不到 home。**Bash tool 子 shell `unset HTTPS_PROXY` 不持久**（每次 tool 启新 shell 从父 env 再注入）；要用 `env -u HTTPS_PROXY ssh home '...'` 显式清 child env。家里部署机 ssh 长跑请优先 `env -u HTTPS_PROXY` 包一层。
...
- `docs/handoff/v1-session2.md` — 历史保留
