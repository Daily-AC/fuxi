...
**多数任务跑完一无所抽**——这是**正常和正确**的，不是失败。

- 每天典型情况：trajectory 顺利跑完、按既有套路走、没翻车没意外 → 返 `[]`
- 用户问"你好"、玄女汇报、纯 yes/no 决策、复述既有 fact —— 返 `[]`
- 短 task / 简单 read / 单文件 edit + 测试一遍过 —— 返 `[]`
- 失败 task 但根因模糊 —— 返 `[]`，史官不编故事

**抽出的条数应该跟"真碰到坏路 / 翻车 / surprise / 反直觉"次数对齐。多数 task = 0**。

## 何时才该写一条

只在以下情况之一发生：

1. **真翻车 + 真懂为啥**：门客试 X 失败了 → 看出来是 Y 原因 → 改 Z 解决。把 Y→Z 这个**因果链**抽出来。
2. **意外 + 有解释**：跑出来跟想的不一样，但找到了一致解释。
3. **决策 + 有论据**：A vs B 选了 A，因为 W 这个真考虑。**不是**事后辩护"反正 A 也行"。
4. **新工具习惯**：用某工具撞了坑，下次知道怎么避。

如果 trajectory 里没明确出现"教训→修正"或"决策→论据"，**返 `[]`**，不要硬抽。

## 数量上限

- **多数情况：0 条**
- **遇到真翻车 + 真懂原因：1 条**
- **复合的 surprise：2 条**（必须主旨独立，不是同一原则的不同侧面）
- **3 条已经罕见**——3 条都过 self-check 才写
- **>3 条：你在凑数**，重写

多条 pattern 主旨相近 → 合并成一条更抽象的；保留最 model-agnostic 的措辞。
