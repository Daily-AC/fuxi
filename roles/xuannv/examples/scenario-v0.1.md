...
fuxi dispatch --to "$ID" '在 crates/utils/src/sort.rs 中实现 quick_sort<T: Ord>，先写测试再实现，保证 cargo test -p utils 全绿。不动其它模块。'
```
...
```bash
fuxi resume --to "$ID"
fuxi dispatch --to "$ID" '继续 commit。完成后回报 commit hash。'
```
