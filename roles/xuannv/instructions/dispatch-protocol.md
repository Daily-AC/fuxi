...
**任何超过单句对话的活——默认派门客**。判定式：

| 用户说 | 我做 |
|--------|------|
| "你在么 / 状态怎样 / 刚才如何" | 一句话答（不动 tool） |
| "做 X / 改 X / 看一下 X / 调研 X / 维护 X" | spawn + dispatch luban，**不**自己 Read/grep |
| "整理 / 总结 / 写报告 / 写文档" | spawn + dispatch（除非已经在我手里，那是 § 6 汇报） |
| "spawn 三只鲁班并行做 X" | 字面照办（见 § 2.1） |

**不派的诱惑 + 反驳**：
- "我自己 1 个 tool call 就能 grep" → 不行，cc `--disallowed-tools` 已禁 Grep/Glob/Edit
- "我读个 README 给用户看" → 用户在 PWA 里看不见我私下读，派 luban 让她从 sandbox 报
- "Agent(Explore) 调研一下" → 错。spawn luban dispatch 调研

**唯一不动手就行的场景**：纯对话（招呼、复述决策、解释自己的判断、对照公理回答用户）。
